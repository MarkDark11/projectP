import pandas as pd

df = pd.read_excel('port.xlsx',sheet_name='Лист2')
df2 = pd.read_excel('anal.xlsx',sheet_name='Лист2')

df = df.drop(df.columns[[11, 12, 13, 14, 15, 16]], axis=1)
df2 = df2.drop(['Пустой столбец1', 'Пустой столбец2', 'Пустой столбец3', 'Пустой столбец4',
                     'Пустой столбец5', 'Пустой столбец6', 'Затраченное время'], axis=1)

df['Дата регистрации'] = pd.to_datetime(df['Дата регистрации'])
df['Дата решения'] = pd.to_datetime(df['Дата решения'])
df['Дата регистрации'] = df['Дата регистрации'].dt.strftime('%d.%m.%Y')
df['Дата решения'] = df['Дата решения'].dt.strftime('%d.%m.%Y')

df2['Дата регистрации'] = pd.to_datetime(df['Дата регистрации'])
df2['Дата завершения заявки'] = pd.to_datetime(df['Дата завершения заявки'])
df2['Дата регистрации'] = df2['Дата регистрации'].dt.strftime('%d.%m.%Y')
df2['Дата завершения заявки'] = df2['Дата завершения заявки'].dt.strftime('%d.%m.%Y')

result_df = pd.concat([df, df2])

# mes
mes_df = result_df.query("Статус != 'Выполнено' and Статус != 'Закрыто' and Статус != 'Отклонено' and Статус != 'Отозвано' "
                         "and Соглашение == 'Договор сопровождения ИУС-ГАЗ'")
mes_df.insert(0, "№", 0)
mes_df.loc[:, '№'] = range(1, len(mes_df) + 1)
mes_df.to_excel('mes.xlsx', index=False)

# epu
epu_df = result_df.query("(Статус != 'Выполнено' and Статус != 'Закрыто' and Статус != 'Отклонено' and Статус != 'Отозвано') "
                         "and (Соглашение == 'Сопровождение ИУСЦИФРА ЕПУ СТМ' or Соглашение == 'Сопровождение ИУСЦИФРА Чек-лист ВДГО' "
                         "or Соглашение == 'Сопровождение пультов управлений')")
epu_df.insert(0, "№", 0)
epu_df.loc[:, '№'] = range(1, len(epu_df) + 1)
epu_df.to_excel('epu.xlsx', index=False)

# olap
olap_df = result_df.query("Статус != 'Выполнено' and Статус != 'Закрыто' and Статус != 'Отклонено' and Статус != 'Отозвано'"
                          "and Статус != 'На тестировании / показ пользователям' and Статус != 'Планируется в релизе' "
                          "and Соглашение != 'Договор сопровождения ИУС-ГАЗ' and Соглашение != 'Внутренние услуги.' "
                          "and Соглашение != 'Договор обслуживания ЕОГ' and Соглашение != 'Договор технического обслуживания СТИ' "
                          "and Соглашение != 'Сопровождение ИТ, коммуникаций и связи' and Соглашение != 'Сопровождение СТИ СКФО' "
                          "and Соглашение != '{Пустые}' and Соглашение != 'Сопровождение ИУСЦИФРА ЕПУ СТМ' "
                          "and Соглашение != 'Сопровождение ИУСЦИФРА Чек-лист ВДГО' and Соглашение != 'Сопровождение пультов управлений'")
olap_df.insert(0, "№", 0)
olap_df.loc[:, '№'] = range(1, len(olap_df) + 1)
olap_df.to_excel('olap.xlsx', index=False)

# sti
sti_df = result_df.query("(Статус != 'Выполнено' and Статус != 'Закрыто' and Статус != 'Отклонено' and Статус != 'Отозвано') "
                         "and (Соглашение == 'Договор технического обслуживания СТИ' or Соглашение == 'Сопровождение СТИ СКФО'"
                         "or Соглашение == 'Сопровождение ИТ, коммуникаций и связи')"
                         "and Организация != 'ООО \"Газпром межрегионгаз ЕЦРК\"'")
sti_df.insert(0, "№", 0)
sti_df.loc[:, '№'] = range(1, len(sti_df) + 1)
sti_df.to_excel('sti.xlsx', index=False)

# eog
eog_df = result_df.query("Статус != 'Выполнено' and Статус != 'Закрыто' and Статус != 'Отклонено' and Статус != 'Отозвано' "
                         "and Соглашение == 'Договор обслуживания ЕОГ'")
eog_df.insert(0, "N", 0)
eog_df.loc[:, '№'] = range(1, len(eog_df) + 1)
eog_df.to_excel('eog.xlsx', index=False)

# ecrk
ecrk_df = result_df.query("Статус != 'Выполнено' and Статус != 'Закрыто' and Статус != 'Отклонено' and Статус != 'Отозвано' "
                         "and (Соглашение == 'Договор технического обслуживания СТИ' or Соглашение == 'Сопровождение СТИ СКФО'"
                         "or Соглашение == 'Сопровождение ИТ, коммуникаций и связи')"
                         "and Организация == 'ООО \"Газпром межрегионгаз ЕЦРК\"'")
ecrk_df.insert(0, "N", 0)
ecrk_df.loc[:, '№'] = range(1, len(ecrk_df) + 1)
ecrk_df.to_excel('ecrk.xlsx', index=False)